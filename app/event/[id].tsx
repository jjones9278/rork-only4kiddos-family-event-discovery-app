...
import { STRIPE_BYPASS_LARAVEL_FOR_TESTING, createTestPaymentIntentDirect } from '@/config/stripe';
...
export default function EventDetailsScreen() {
  ...
  const isFavorite = useIsFavorite(id as string, event?.isFavorite ?? false);
  ...
  const handleBooking = async () => {
    ...
    // DEV-ONLY bypass: skip Laravel and hit Stripe directly to test the
    // Payment Sheet UI without depending on the backend booking flow.
    if (STRIPE_BYPASS_LARAVEL_FOR_TESTING) {
      try {
        const amount = (event.price || 1) * selectedChildren.length;
        const pi = await createTestPaymentIntentDirect(amount);
        router.push({
        ...
        toastError('Stripe Test Failed', e?.message || 'Unable to create test PaymentIntent.');
      }
      return;
    }
    ...
    try {
      ...
      // Free event: server already confirmed.
      toastSuccess(
        'Booking Confirmed!',
      ...
      toastError('Booking Failed', error?.message || 'Unable to complete booking. Please try again.');
    }
  };
